...
for i in stock:
    ...
    for table in x.find_all("table"):
        tableandheading.append(headings[head_count])
        ...
        tableandheading.append(table_data_m)

    header=[]
    header.append(tableandheading[0])
    df2sam =  pd.DataFrame(header)
    df = pd.DataFrame(tableandheading[1])
    frames = [df2sam, df]
