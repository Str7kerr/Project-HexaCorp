...
root = Tk()
cxn = sqlite3.connect('cricstats.db')
...
myText1.grid(row=0, column=4, columnspan=2, padx=42)
...
myButton1 = Button(root, text="Single",bg = '#0096FF', fg = 'white', command=openFile)
myButton1.grid(row=1, column=4, pady=5)
myButton2 = Button(root, text="Multiple",bg = '#0096FF', fg = 'white', command=openFiles)
...
def query():
    ...
    for r in r_set:
        for j in range(len(r)):
            if j is 5:
                e = Entry(relief='solid',width=10, font=('Arial', 10, 'bold'))
...
show_button = Button(root, text="View",  bg = '#0096FF', fg = 'white',command=query)
show_button.grid(row=2, column=4, pady=5)
...
delete_button = Button(root, text='Delete',bg = '#0096FF', fg = 'white', command=delete)
delete_button.grid(row=2, column=5)
